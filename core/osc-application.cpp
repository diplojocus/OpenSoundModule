...
//----- IP ADRESSES
IPAddress computerIPAddress = IPAddress(192,168,2,174);	// put the IP address of your computer here
...
{
	if (mess.size() == 2) {
		int thisLed = (int) mess.getFloat(0);
		int thisStatus = (int) mess.getFloat(1);
		Serial.println("Set LED-" + String(thisLed) + " " + String(thisStatus));
...
void loop()
{
	...
	if (manageMessages == 1 && manageBundles == 0) {
        ...
        Serial.println("Received osc bundle");
